...
from numpy import sin, cos, deg2rad, rad2deg, arctan2, sqrt, arcsin, tan
from numpy.linalg import multi_dot, norm

# Local application imports
from myastro import timeutil as tc
from myastro import coord as co
#from myastro.orbit import calc_M, elliptic_orbit, next_E

#from myastro import planets as pl
#from myastro import orbit as ob

from myastro.timeutil import JD_J2000, JD_B1950, CENTURY
from myastro.coord import obliquity, Coord, EQUAT2_TYPE, polarFcartesian, mtx_equatFeclip,mtx_gauss_vectors,mtx_eclip_prec, prec_mtx_equat
from myastro.util import pow, memoize
...
TWOPI = 2*np.pi
PI = np.pi
VSOP87_DATA_DIR_NAME= '/home/anybody/wsl_projs/vsop87/'
...
def h_xyz_eclip_eqxdate(name, jde):
    sfx = name.lower()[:3]
    fn=VSOP87_DATA_DIR_NAME+'VSOP87C.'+sfx
    x, y, z = calc_series(fn, jde,['X','Y','Z'])    
    return np.array([x,y,z])

def h_xyz_eclip_j2000(name, jde):
    sfx = name.lower()[:3]
    fn=VSOP87_DATA_DIR_NAME+'VSOP87A.'+sfx
    x, y, z = calc_series(fn, jde,['X','Y','Z'])    
    return np.array([x,y,z])
...
def h_rlb_eclip_eqxdate(name, jde, tofk5=False):
    ...
    sfx = name.lower()[:3]
    fn=VSOP87_DATA_DIR_NAME+'VSOP87D.'+sfx
...
def h_rlb_eclip_j2000(name, jde):
    sfx = name.lower()[:3]
    fn=VSOP87_DATA_DIR_NAME+'VSOP87B.'+sfx
    r, l, b = calc_series(fn, jde,['R','L','B'])    
