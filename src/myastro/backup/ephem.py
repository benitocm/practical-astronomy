...
from myastro.orbit import calc_M, elliptic_orbit, h_xyz_eclip_keplerian_orbit, next_E, parabolic_orbit, hyperpolic_orbit
...
def calc_eph_minor_body_perturbed (body, eph , include_osc=False):
    ...
    if ((M < M0) and (np.abs(1.0-body.e) < 0.1)) or isclose(body.e, 1.0,  abs_tol=1e-08) :
        ...
        xyz0, vxyz0 = parabolic_orbit(body.tp_mjd, body.q, body.e, initial_mjd, 50)
    elif body.e < 1.0 :
        logger.warning(f'Doing elliptic orbit for e: {body.e}')
        xyz0, vxyz0 = elliptic_orbit(ob.next_E, M, body.a, body.e)
    else :
        logger.warning(f'Doing hyperbolic orbit for e: {body.e}')
        xyz0, vxyz0 =  hyperpolic_orbit (body.tp_mjd, ob.next_H, np.abs(body.a), body.e, initial_mjd)
    ...
    for idx, clock_mjd in enumerate(SOL_T) :  
        ...
        oscul_elms = ob.calc_osculating_orb_elmts(h_xyz, h_vxyz, clock_mjd, eph.eqx_name)
...
def calc_eph_comet(body, eph):
    ...
    for clock_mjd in ut.frange(eph.from_mjd, eph.to_mjd, eph.step):        
        ...
        g_rlb_eclipt_T = pipe (g_rlb_eclip, cartesianFpolar, mtx_eclip_prec(T,T_J2000).dot, polarFcartesian)
        row['Sun(dg)'] = f"{rad2deg(g_rlb_eclipt_T[1]):03.1f}"
...
def test_calc_eph_comet():
    HALLEY_1950 = CometElms(name="1P/Halley",
                epoch_name=None ,
                q =  0.5870992 ,
                e = 0.9672725 ,
                i_dg = 162.23932 ,
